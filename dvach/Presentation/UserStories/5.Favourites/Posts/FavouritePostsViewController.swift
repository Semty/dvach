...
protocol FavouritePostsView {
    func updateTable()
}

final class FavouritePostsViewController: UIViewController {
    
    // Dependencies
    private let presenter: IFavouritePostsPresenter
    ...
    private lazy var placeholderView: UIView = {
        let label = UILabel()
        label.text = "У вас пока нет любимых постов"
    ...
    init() {
        let presenter = FavouritePostsPresenter()
...
extension FavouritePostsViewController: FavouritePostsView {
...
extension FavouritePostsViewController: UITableViewDelegate {
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        presenter.didSelectPost(index: indexPath.row)
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            presenter.didRemovePost(index: indexPath.row)
        }
    }
}
