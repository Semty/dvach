...
public extension PopView {
  ...
  private func entranceScale(completion: @escaping () -> Void) {
    ...
    UIView.animate(withDuration: animationIn, delay: delayIn, usingSpringWithDamping: 0.8, initialSpringVelocity: 1.25, options: [.curveEaseInOut, .beginFromCurrentState], animations: {
      self.transform = .identity
      self.backgroundMask?.alpha = 1
    }) { (_) in
      completion()
