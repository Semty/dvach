...
/// Enum that specifies the direction of the popview
public enum PopViewDirection {
  /// Up, the popview will appear above the element, arrow pointing down
  case up
  /// Down, the popview will appear below the element, arrow pointing up
  case down
  /// Left, the popview will appear on the left of the element, arrow pointing right
  case left
  /// Right, the popview will appear on the right of the element, arrow pointing left
  case right
  /// None, the popview will appear above the element with no arrow
  case none
}

/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the popview.
 
 - `scale`: The popview scales from 0% to 100%
 - `transitions`: The popview moves in position from the edge of the screen
 - `fadeIn`: The popview fade in
 - `custom`: The Animation is provided by the user
 - `none`: No Animation
 */
public enum PopViewEntranceAnimation {
  /// The popview scales from 0% to 100%
  case scale
  /// The popview moves in position from the edge of the screen
  case transition
  /// The popview fades in
  case fadeIn
...
/** Enum that specifies the type of entrance animation. Entrance animations are performed while showing the popview.
 ...
 - `scale`: The popview scales from 100% to 0%
 - `fadeOut`: The popview fade out
 - `custom`: The Animation is provided by the user
 - `none`: No Animation
 */
public enum PopViewExitAnimation {
  /// The popview scales from 100% to 0%
  case scale
  /// The popview fades out
  case fadeOut
  /// The Animation is provided by the user
  case custom
  /// No Animation
...
/** Enum that specifies the type of action animation. Action animations are performed after the popview is visible and the entrance animation completed.
 
 - `bounce(offset: CGFloat?)`: The popview bounces following its direction. The bounce offset can be provided optionally
 - `float(offset: CGFloat?)`: The popview floats in place. The float offset can be provided optionally
 - `pulse(offset: CGFloat?)`: The popview pulsates by changing its size. The maximum amount of pulse increase can be provided optionally
 - `none`: No animation
 */
public enum PopViewActionAnimation {
  /// The popview bounces following its direction. The bounce offset can be provided optionally
  case bounce(CGFloat?)
  /// The popview floats in place. The float offset can be provided optionally. Defaults to 8 points
  case float(offsetX: CGFloat?, offsetY: CGFloat?)
  /// The popview pulsates by changing its size. The maximum amount of pulse increase can be provided optionally. Defaults to 1.1 (110% of the original size)
  case pulse(CGFloat?)
  /// No animation
  case none
}
...
open class PopView: UIView {
  /// The text displayed by the popview. Can be updated once the popview is visible
  ...
  /// The `UIFont` used in the popview's text
  ...
  /// The `UIColor` for the popview's background
  @objc open dynamic var bubbleColor = UIColor.red
  /// The `UIColor` for the popview's bordedr
  @objc open dynamic var borderColor = UIColor.clear
  /// The width for the popview's border
  @objc open dynamic var borderWidth = CGFloat(0.0)
  /// The `Double` with the popview's border radius
  @objc open dynamic var cornerRadius = CGFloat(4.0)
  /// The `BOOL` that determines wether the popview is rounded. If set to `true` the radius will equal `frame.height / 2`
  @objc open dynamic var isRounded = false
  /// The `UIColor` with the popview's shadow color
  @objc open dynamic var shadowColor: UIColor = .clear
  /// The `CGSize` with the popview's shadow offset
  @objc open dynamic var shadowOffset: CGSize = .zero
  /// The `Float` with the popview's shadow radius
  @objc open dynamic var shadowRadius: Float = 0
  /// The `Float` with the popview's shadow opacity
  @objc open dynamic var shadowOpacity: Float = 0
  /// Holds the offset between the popview and origin
  ...
  /// Holds the enum with the type of entrance animation (triggered once the popview is shown)
  open var entranceAnimation = PopViewEntranceAnimation.scale
  /// Holds the enum with the type of exit animation (triggered once the popview is dismissed)
  open var exitAnimation = PopViewExitAnimation.scale
  /// Holds the enum with the type of action animation (triggered once the popview is shown)
  open var actionAnimation = PopViewActionAnimation.none
  ...
  /// Holds the readonly BOOL with the popview visiblity. The popview is considered visible as soon as
  ...
  /// A boolean value that determines whether the popview is dismissed on tap.
  @objc open dynamic var shouldDismissOnTap = true
  /// A boolean value that determines whether to dismiss when tapping outside the popview.
  @objc open dynamic var shouldDismissOnTapOutside = true
  /// A boolean value that determines whether to consider the originating frame as part of the popview,
  /// i.e wether to call the `tapHandler` or the `tapOutsideHandler` when the tap occurs in the `from` frame.
  @objc open dynamic var shouldConsiderOriginatingFrameAsPopView = false
  /// A boolean value that determines whether to dismiss when swiping outside the popview.
  @objc open dynamic var shouldDismissOnSwipeOutside = false
  /// A boolean value that determines if the action animation should start automatically when the popview is shown
  @objc open dynamic var startActionAnimationOnShow = true
  /// A direction that determines what swipe direction to dismiss when swiping outside the popview.
  ...
  /// A block that will be fired when the user taps the popview.
  open var tapHandler: ((PopView) -> Void)?
  /// A block that will be fired when the user taps outside the popview.
  open var tapOutsideHandler: ((PopView) -> Void)?
  /// A block that will be fired when the user swipes outside the popview.
  open var swipeOutsideHandler: ((PopView) -> Void)?
  /// A block that will be fired when the popview appears.
  open var appearHandler: ((PopView) -> Void)?
  /// A block that will be fired when the popview is dismissed.
  open var dismissHandler: ((PopView) -> Void)?
  /// A block that handles the entrance animation of the popview. Should be provided
  /// when using a `PopViewActionAnimationCustom` entrance animation type.
  /// Please note that the popview will be automatically added as a subview before firing the block
  /// Remember to call the completion block provided
  open var entranceAnimationHandler: ((@escaping () -> Void) -> Void)?
  /// A block block that handles the exit animation of the popview. Should be provided
  /// when using a `AMPopViewActionAnimationCustom` exit animation type.
  /// Remember to call the completion block provided
  ...
  /// A read only reference to the view containing the popview
  open private(set) weak var containerView: UIView?
  /// The direction from which the popview is shown. Read only.
  open private(set) var direction = PopViewDirection.none
  /// Holds the readonly BOOL with the popview animation state.
  open private(set) var isAnimating: Bool = false
  /// The view that dims the background (including the button that triggered PopView.
  ...
  /// Setup a popview oriented vertically (direction .up or .down). Returns the bubble frame and the arrow position
  ///
  ...
  /// Setup a popview oriented horizontally (direction .left or .right). Returns the bubble frame and the arrow position
  ///
  /// - Returns: a tuple with the bubble frame and the arrow position
  ...
  internal func rectContained(rect: CGRect) -> CGRect {
    ...
    // The `.none` positioning implies a rect with the origin in the middle of the popview
  ...
  fileprivate func setup() {
    ...
    if tapGestureRecognizer == nil {
      tapGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(PopView.handleTap(_:)))
      tapGestureRecognizer?.cancelsTouchesInView = false
      self.addGestureRecognizer(tapGestureRecognizer!)
    }
    if shouldDismissOnTapOutside && tapRemoveGestureRecognizer == nil {
      tapRemoveGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(PopView.handleTapOutside(_:)))
    }
    if shouldDismissOnSwipeOutside && swipeGestureRecognizer == nil {
      swipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(PopView.handleSwipeOutside(_:)))
      swipeGestureRecognizer?.direction = swipeRemoveGestureDirection
    }
    
    if isApplicationInBackground == nil {
      NotificationCenter.default.addObserver(self, selector: #selector(PopView.handleApplicationActive), name: UIApplication.didBecomeActiveNotification, object: nil)
      NotificationCenter.default.addObserver(self, selector: #selector(PopView.handleApplicationResignActive), name: UIApplication.willResignActiveNotification, object: nil)
    }
  }
  ...
  open override func draw(_ rect: CGRect) {
    ...
    let path = PopView.pathWith(rect: rect, frame: frame, direction: direction, arrowSize: arrowSize, arrowPosition: arrowPosition, arrowRadius: arrowRadius, borderWidth: borderWidth, radius: cornerRadius)
  ...
  /// Shows an animated popview in a given view, from a given rectangle. The property `isVisible` will be `true` as soon as the popview is added to the given view.
  ///
  /// - Parameters:
  ///   - text: The text to display
  ///   - direction: The direction of the popview in relation to the element that generates it
  ///   - maxWidth: The maximum width of the popview. If the popview won't fit in the given space, this will be overridden.
  ///   - view: The view that will hold the popview as a subview.
  ///   - frame: The originating frame. The popview's arrow will point to the center of this frame.
  ///   - duration: Optional time interval that determines when the popview will self-dismiss.
  open func show(text: String, direction: PopViewDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
    resetView()
  ...
  ///   - frame: The originating frame. The popview's arrow will point to the center of this frame.
  ///   - duration: Optional time interval that determines when the popview will self-dismiss.
  open func show(attributedText: NSAttributedString, direction: PopViewDirection, maxWidth: CGFloat, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
    resetView()
  ...
  ///   - frame: The originating frame. The popview's arrow will point to the center of this frame.
  ///   - duration: Optional time interval that determines when the popview will self-dismiss.
  open func show(customView: UIView, direction: PopViewDirection, in view: UIView, from frame: CGRect, duration: TimeInterval? = nil) {
    resetView()
  ...
  ///   - view: The view that will hold the popview as a subview.
  ///   - frame: The originating frame. The popview's arrow will point to the center of this frame.
  ///   - parent: The controller that holds the view that will hold the popview. Needed as SwiftUI views have to be embed in a child UIHostingController.
  ///   - duration: Optional time interval that determines when the popview will self-dismiss.
  @available(iOS 13.0, *)
  open func show<V: View>(rootView: V, direction: PopViewDirection, in view: UIView, from frame: CGRect, parent: UIViewController, duration: TimeInterval? = nil) {
    resetView()
  ...
  /// Hides the popview and removes it from the view. The property `isVisible` will be set to `false` when the animation is complete and the popview is removed from the parent view.
  ///
  ...
  /// Makes the popview perform the action indefinitely. The action animation calls for the user's attention after the popview is shown
  open func startActionAnimation() {
    performActionAnimation()
  }
  
  /// Stops the popview action animation. Does nothing if the popview wasn't animating in the first place.
  ///
  ...
  fileprivate func updateBubble() {
    stopActionAnimation {
      UIView.animate(withDuration: 0.2, delay: 0, options: [.transitionCrossDissolve, .beginFromCurrentState], animations: {
        ...
        let path = PopView.pathWith(rect: self.frame, frame: self.frame, direction: self.direction, arrowSize: self.arrowSize, arrowPosition: self.arrowPosition, arrowRadius: self.arrowRadius, borderWidth: self.borderWidth, radius: self.cornerRadius)
  ...
  fileprivate func show(duration: TimeInterval? = nil) {
    ...
    performEntranceAnimation {
      ...
      if let duration = duration {
        self.dismissTimer = Timer.scheduledTimer(timeInterval: duration, target: self, selector: #selector(PopView.hide), userInfo: nil, repeats: false)
      }
    }
  }
  ...
  @objc fileprivate func handleTapOutside(_ gesture: UITapGestureRecognizer) {
    ...
    if shouldConsiderOriginatingFrameAsPopView && from.contains(gesture.location(in: containerView)) {
      tapHandler?(self)
    } else {
  ...
  fileprivate func bounceAnimation(offset: CGFloat) {
    ...
    UIView.animate(withDuration: actionAnimationIn / 10, delay: actionDelayIn, options: [.curveEaseIn, .allowUserInteraction, .beginFromCurrentState], animations: {
      ...
      if completed {
        UIView.animate(withDuration: self.actionAnimationIn - self.actionAnimationIn / 10, delay: 0, usingSpringWithDamping: 0.4, initialSpringVelocity: 0.2, options: [.allowUserInteraction, .beginFromCurrentState], animations: {
          self.transform = .identity
        }, completion: { (done) in
