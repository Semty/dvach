//
//  WriteLockableSynchronizedArray.swift
//  dvach
//
//  Created by Ruslan Timchenko on 20/07/2019.
...
public class WriteLockableSynchronizedArray<Element> {
    ...
    fileprivate let queue = DispatchQueue(label: "com.ruslantimchenko.WriteLockableSynchronizedArray", attributes: .concurrent)
...
public extension WriteLockableSynchronizedArray {
    ...
    func index(where predicate: (Element) -> Bool) -> Int? {
        var result: Int?
        queue.sync { result = self.internalArray.firstIndex(where: predicate) }
        return result
    }
...
public extension WriteLockableSynchronizedArray {
    ...
    func remove(where predicate: @escaping (Element) -> Bool, completion: ((Element) -> Void)? = nil) {
        let op = {
            guard let index = self.internalArray.firstIndex(where: predicate) else { return }
            let element = self.internalArray.remove(at: index)
            DispatchQueue.main.async {
                completion?(element)
            }
